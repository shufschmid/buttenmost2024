...
export default defineEventHandler(async (event) => {
  ...
  const specimen = isTrue(query.preview);
  ...
  try {
    ...
    console.log("etikette_brother Airtable-Fehler", e?.message ?? e);
    return Response.json({ error: "Airtable: " + (e?.message ?? e) }, { status: 502 });
  }
  const f = record.fields;

  // 2. Absender-Bild (Menge + Logo), identisch zu etikette.js
  const liter = f.Menge;
  const abholen = f.vertrieb;
  const width = 470;
  const height = 250;
  ...
  const fnt = PImage.registerFont("public/sourcesanspro-regular.ttf", "Source Sans Pro");
  fnt.loadSync();

  ctx.fillStyle = "#000000";
  ctx.font = "100pt 'Source Sans Pro'";
  ctx.fillText(String(liter ?? ""), 10, 80);
  if (abholen === "Abholung") {
    ctx.fillText(">", 50, 210);
    ctx.fillText("=", 10, 210);
    ctx.fillText("|", 0, 200);
  ...
  const logoImg = await PImage.decodePNGFromStream(
    Readable.from(Buffer.from(buttenmostlogo, "base64"))
  );
  ctx.drawImage(logoImg, 110, 0);

  const logoPng = await encodeToBuffer((sink) => PImage.encodePNGToStream(img1, sink));
  const b64 = logoPng.toString("base64");

  // 3. Post: Token holen, Etikette als PNG (A6, 300 dpi) erzeugen
  let label;
  let sendungsnummer = "";
  try {
    ...
    const res = await axios.request({
      ...
      data: {
        ...
        labelDefinition: {
          ...
          imageFileType: "PNG", // verlustfrei; JPG wird von der Post fuer Barcodes nicht empfohlen
          imageResolution: 300, // 1 Pixel = 1 Druckpunkt am QL-1110NWB
          printPreview: specimen, // true -> SPECIMEN-Aufdruck
        },
        item: {
          itemID: f.id,
          recipient: {
    ...
    sendungsnummer = res.data?.item?.identCode ?? "";
    const postErrors = res.data?.item?.errors;
    if (postErrors?.length) {
      const err = new Error("Post meldet Fehler");
      err.postErrors = postErrors;
      throw err;
    }
  } catch (error) {
    const status = error.response?.status;
    const details = error.response?.data ?? error.postErrors ?? error.message;
    console.log("etikette_brother Post-Fehler", status, JSON.stringify(details));
    return Response.json({ error: "Post-API Fehler", status, details }, { status: 502 });
  }

  if (Array.isArray(label)) label = label[0];
  if (!label) {
    return Response.json({ error: "Kein Bild generiert" }, { status: 502 });
  }

  // 4. PNG -> Brother-Raster (.bin) + Vorschau
  ...
  try {
    ...
    return Response.json(
      { error: "Rasterung fehlgeschlagen: " + (e?.message ?? e) },
      { status: 500 }
    );
  }
  console.log(
  ...
  if (!specimen) {
    try {
      await base(TABLE).update(id, { Status: "Etikette", Sendungsnummer: sendungsnummer });
      console.log(sendungsnummer, "Status Etikette und Sendungsnummer gesetzt");
    } catch (e) {
      warning = "Airtable-Update fehlgeschlagen: " + (e?.message ?? e);
      console.log("etikette_brother", warning);
    }
  }
  if (raster.croppedDark > 0) {
    warning +=
      (warning ? " " : "") +
      `Achtung: ${raster.croppedDark} schwarze Punkte im weggeschnittenen Rand.`;
  }

  const filename = `${specimen ? "TEST-" : ""}${sendungsnummer || "etikette"}.bin`;
  if (raw) {
  ...
  return Response.json(
    {
      ...
      srcHeight: raster.srcHeight,
      rotated: raster.rotated,
